...
def GetVesshancementMask(img):

    # step1. Normalization
    # step2. Franji vess enhancement
    # step3. Dilation
    # step4. Threshold

    img_normal = cv2.normalize(img.astype('double'), None, 0.0, 1.0, cv2.NORM_MINMAX) 
    img_vess = FrangiFilter2D(img_normal)
    img_vess = np.uint8(img_vess*(10000))*255
    NpKernel = np.uint8(np.ones((3,3)))
    img_vess = cv2.dilate(img_vess, NpKernel)
    _, img_vess_mask = cv2.threshold(img_vess, 128, 1, cv2.THRESH_BINARY)
    return img_vess_mask

if __name__ == '__main__':

    img_name = '015_axial0079_lung.png'
    img = cv2.imread(img_name, 0)
    mask = GetVesshancementMask(img)
    cv2.imwrite('015_axial0079_vess.jpg', mask)
